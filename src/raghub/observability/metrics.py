...
class PrometheusMetrics:
    def __init__(self, app: Any | None = None) -> None:
        ...
        def _safe_histogram(name: str, desc: str, buckets: list[float]) -> Histogram | None:
            existing = REGISTRY._names_to_collectors.get(_metric_name(name))
            if existing is not None:
                return existing
            return Histogram(name, desc, buckets=buckets, registry=REGISTRY)

        def _safe_counter(name: str, desc: str, labels: list[str] | None = None) -> Counter | None:
            existing = REGISTRY._names_to_collectors.get(name)
            if existing is not None:
                return existing
            return Counter(name, desc, labels or [], registry=REGISTRY)

        self.query_duration = _safe_histogram(
            "raghub_query_duration_ms",
            "Query execution duration in milliseconds",
            [10, 50, 100, 250, 500, 1000, 2500, 5000],
        )
        self.ingestion_duration = _safe_histogram(
            "raghub_ingestion_duration_ms",
            "Ingestion duration in milliseconds",
            [50, 100, 250, 500, 1000, 2500, 5000, 10000],
        )
        self.auth_duration = _safe_histogram(
            "raghub_auth_duration_ms",
            "Authentication duration in milliseconds",
            [5, 10, 25, 50, 100, 250, 500],
        )
        self.auth_total = _safe_counter("raghub_auth_total", "Total authentication attempts", ["success"])
        self.error_total = _safe_counter("raghub_error_total", "Total errors", ["error_type"])
        if app is not None:
