...
* The :class:`FeatureHashingEmbedder` is deterministic — the same input always
...
from raghub.embedder import FeatureHashingEmbedder, build_embedder
...
class TestHashingEmbeddingProvider:
    ...
    def test_model_name_is_recorded(self) -> None:
        provider = FeatureHashingEmbedder(dimension=8, model_name="my-model")
...
class TestDimensionContract:
    ...
    def test_hashing_actually_returns_requested_dimension(self, dimension: int) -> None:
        provider = FeatureHashingEmbedder(dimension=dimension)
        vec = provider.embed_text("test")
...
class TestBuildEmbedder:
    def test_hashing_bge_dispatches_to_hashing(self) -> None:
        ...
        assert isinstance(provider, FeatureHashingEmbedder)
