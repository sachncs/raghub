    ...
    def register(app: typer.Typer) -> None:
        ...
        def export_cmd(
            ...
            if store is None:
                ...
                raise typer.Exit(code=1)
            if tenant is not None:
                records = asyncio.run(store.list_for_tenant(tenant))
            else:
                # Empty tenant_id string matches every row in the SQL
                # implementation; iterate via a synthetic empty filter
                # and combine with the no-filter case by reusing "".
                records = []
                # Use the unfiltered path: tenant_id=None returns all.
                records = asyncio.run(store.list_for_tenant(""))
