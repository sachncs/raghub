...
def build_embedding_provider(
    ...
    if "litellm" in name or any(
        name.startswith(prefix) for prefix in ("openai/", "cohere/", "voyage/", "azure/")
    ):
        from .litellm import LiteLLMEmbeddingProvider

        return LiteLLMEmbeddingProvider(model=model_name, dimension=dimension, api_key=api_key)
