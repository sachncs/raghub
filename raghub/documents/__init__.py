...
This module bundles every concern related to a single document: how
it is parsed, how its text is chunked, how its lifecycle state
advances, how versions are minted, and how raw bytes are converted
into canonical :class:`KnowledgeBundle` objects. Callers usually
orchestrate these pieces via :mod:`raghub.ingestion` rather than
touching this module directly.

The :func:`select_converter_for_path` / :func:`convert_path` helpers
route on-disk files to :class:`MarkerConverter` (PDFs) or
:class:`PlainTextConverter` (everything else). Markdown normalisation
(:func:`normalise_markdown`) and the structural
:func:`markdown_to_document_blocks` parser are shared by both.
"""
...
# ---------------------------------------------------------------------------
# Converters (Marker / PlainText / Markdown / Directory)
# ---------------------------------------------------------------------------
