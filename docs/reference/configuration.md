...
the [`raghub.RAG`](https://github.com/sachncs/revex/blob/main/revex/api/rag.py) facade and the legacy
`RagApplication`. It is a Python `@dataclass(slots=True)`;
...
| `REVEX_USERS` | Demo-user JSON override (JSON object of email → `{password, companies, is_admin}`) |
| `REVEX_FINANCEBENCH_CACHE` | Local Finance cache directory |
...
Generated by `raghub init -o revex.yaml`:
