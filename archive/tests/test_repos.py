...
def test_chunk_store_initialize_brings_store_online(memory_store: MemoryStore) -> None:
    ...
    _run(repo.upsert(chunk, [0.1, 0.2, 0.3]))
    assert memory_store.health()["chunks"] == 1


def test_chunk_store_upsert_single_chunk_persists(memory_store: MemoryStore) -> None:
    """``ChunkStore.upsert`` writes a single chunk so it is reachable by id and search."""
    ...
    hits = memory_store.search(vector=[0.1, 0.2, 0.3], top_k=5)
    assert len(hits) == 1
...
def test_chunk_store_delete_removes_chunk(memory_store: MemoryStore) -> None:
    """``ChunkStore.delete`` removes the chunk so a follow-up search returns nothing."""
...
def test_chunk_store_search_returns_results(memory_store: MemoryStore) -> None:
    ...
    _run(repo.upsert(chunk, [1.0, 0.0, 0.0]))

    results = memory_store.search(vector=[1.0, 0.0, 0.0], top_k=5)
    assert len(results) == 1
    assert results[0]["chunk_id"] == "s1"
    assert results[0]["score"] > 0.99  # identical vector → cosine 1.0


def test_store_optimize_is_safe(memory_store: MemoryStore) -> None:
    """``Store.optimize`` runs against a populated store without raising or losing data."""

    repo = ChunkStore(memory_store)
    chunk = _make_chunk("opt-1")
    _run(repo.upsert(chunk, [0.1, 0.2, 0.3]))

    memory_store.optimize()
...
def test_store_health_reports_store_state(memory_store: MemoryStore) -> None:
    """``Store.health`` reports status and chunk count."""

    repo = ChunkStore(memory_store)
    _run(repo.upsert(_make_chunk("h1"), [0.1, 0.2, 0.3]))
    _run(repo.upsert(_make_chunk("h2"), [0.4, 0.5, 0.6]))

    h = memory_store.health()
    assert h["status"] == "ok"
    assert h["chunks"] == 2
